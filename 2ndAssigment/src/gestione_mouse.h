...
void mouse(int button, int state, int x, int y)
{
	...
	case GLUT_RIGHT_BUTTON:
		...
		if (state == GLUT_DOWN /*&& glutGetModifiers() == GLUT_ACTIVE_CTRL*/)
		{
			...
			// selected_obj = -1;
			selected_meshobj = -1;
			float closest_MeshObj_intersection = 0.0f;
			for (int j = 0; j < ScenaObj.size(); j++)
			{
				float t_dist = 0.0f;
				for (int k = 0; k < ScenaObj[j].size(); k++)
				{
					if (ray_sphere(ViewSetup.position, ray_wor, ScenaObj[j][k].ancora_world, raggio_sfera, &t_dist))
					{
						if (selected_meshobj == -1 || t_dist < closest_MeshObj_intersection)
						{
							selected_meshobj = j;
							closest_MeshObj_intersection = t_dist;
						}
					}
				}
			}
